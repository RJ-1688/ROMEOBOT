...
from RomeoBot.sql.gban_sql import is_gbanned, gbaner, ungbaner, all_gbanned
from RomeoBot.sql.gvar_sql import gvarstat
from RomeoBot.sql import gmute_sql as gsql
from . import *
...
async def _(event):
    ...
    if a:
        ...
        gbpic = cjb
    gmsg = f"[{name}](tg://user?id={userid}) **𝐁𝐞𝐭𝐚 𝐑𝐚𝐡𝐧𝐞 𝐝𝐨 𝐓𝐮𝐦𝐬𝐞 𝐧𝐚 𝐡𝐨 𝐩𝐚𝐲𝐞𝐠𝐚 😏**\n\n📍 Added to Gban Watch!!\n**🔰 Total Chats :**  `{chats}`"
    if reason != "":
        gmsg += f"\n**🔰 Reason :**  `{reason}`"
